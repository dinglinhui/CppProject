...
void show_ip_parts(const std::string& ip) {
	...
	if (valid) {
		std::cout << "IP: " << result[1] << ":" << result[2] << ":" << result[3]
				<< ":" << result[4] << std::endl;
	}
}

void replace(const std::string& text, const std::regex &pattern,
...
void extract(const std::string& s, const std::regex &pattern) {
	...
	for (std::sregex_token_iterator i(s.begin(), s.end(), pattern); i != end;
			++i) {
		std::cout << *i << std::endl;
	}
	std::cout << std::endl;
...
void test_regex() {
	...
	std::cout << email1 << " : "
			<< (is_email_valid(email1) ? "valid" : "invalid") << std::endl;
	std::cout << email2 << " : "
			<< (is_email_valid(email2) ? "valid" : "invalid") << std::endl;
	std::cout << email3 << " : "
			<< (is_email_valid(email3) ? "valid" : "invalid") << std::endl;
	std::cout << email4 << " : "
			<< (is_email_valid(email4) ? "valid" : "invalid") << std::endl;
	...
	std::cout << date1 << " -> " << std::regex_replace(date1, pattern, replace)
			<< std::endl;
	std::cout << date2 << " -> " << std::regex_replace(date2, pattern, replace)
			<< std::endl;
	std::cout << std::endl;

	/***************************************************************/
	std::string s1 =
			"Calls to new must be followed by delete. Calling simply new results in a leak!";
