...
void manager() {
    ...
    std::cout << "Manager: I would need to know if the car engine is ok." << std::endl;
    ...
    if (answer.get()) {
        std::cout << "Manager: ensures the client that the engine is ok.\n" << "Manager: negotiates the contract for selling the car." << std::endl;
    } else {
...
void test_thread() {
...

    {
        ...
        auto countEven = [&increment](const std::vector<int>& numbers, int& numEven) {
