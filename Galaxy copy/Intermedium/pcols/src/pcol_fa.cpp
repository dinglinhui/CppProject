...
static int CPrint_DI_TIME(EM_DATA* pData, char* szOut) {
	...
	sprintf(szOut, "%02d-%02d-%02d %02d:%02d", pFA->year, pFA->month, pFA->day, pFA->hour, pFA->minute);
	return 0;
}
...
static int CPrint_DI04000101(EM_DATA* pData, char* szOut) {
	...
	sprintf(szOut, "%02d-%02d-%02d %02d", pFA->year, pFA->month, pFA->day, pFA->week);
	return 0;
}
...
static int CFA_DI03300501(void *pBody, BYTE *lpBuf) {
	...
	for (BYTE i = 0; i < LINYANG_EM_MAX_TIMETABLE; i++) {
		nPos += CFA_DI04010001(&lpBuf[nPos], (BYTE *) &pFA->DT040100FF.DI0401000X[i]);
	}

	for (BYTE i = 0; i < LINYANG_EM_MAX_TIMETABLE; i++) {
		nPos += CFA_DI04010001(&lpBuf[nPos], (BYTE *) &pFA->DT040200FF.DI0401000X[i]);
	}
...
static int CFP_DI100XFF0X(BYTE *lpBuf, void *pSrc) {
	...
	for (BYTE i = 0; i < 3; i++) {
		nPos += CFP_DI_START_LOSTV_PHASE((BYTE*) &lpBuf[nPos], &pFA->DT_START_LV_PHASE[i]);
	}
	...
	for (BYTE i = 0; i < 3; i++) {
		nPos += CFP_DI_END_LOSTV_PHASE((BYTE*) &lpBuf[nPos], &pFA->DT_START_LV_PHASE[i]);
	}
...
static int CFA_DI100XFF0X(void *pBody, BYTE *lpBuf) {
	...
	for (BYTE i = 0; i < 3; i++) {
		nPos += CFA_DI_START_LOSTV_PHASE(&pFA->DT_START_LV_PHASE[i], (BYTE*) &lpBuf[nPos]);
	}
	...
	for (BYTE i = 0; i < 3; i++) {
		nPos += CFA_DI_END_LOSTV_PHASE(&pFA->DT_START_LV_PHASE[i], (BYTE*) &lpBuf[nPos]);
	}
	//
	return nPos;
}
...
static int CFP_DI1400FF0X(BYTE *lpBuf, void *pSrc) {
	...
	for (BYTE i = 0; i < 3; i++) {
		nPos += CFP_DI_LOSTV_TOTAL((BYTE*) &lpBuf[nPos], &pFA->DT_STR_PS_ERR_PHASE[i]);
	}

	nPos += CFP_DI_TIMEYS((BYTE*) &lpBuf[nPos], &pFA->DT_END_PS_ERR_TIME);
	nPos += CFP_DI_LOSTV_TOTAL((BYTE*) &lpBuf[nPos], &pFA->DT_END_PS_ERR_TOTAL);
	for (BYTE i = 0; i < 3; i++) {
		nPos += CFP_DI_LOSTV_TOTAL((BYTE*) &lpBuf[nPos], &pFA->DT_END_PS_ERR_PHASE[i]);
	}
...
static int CFA_DI1400FF0X(void *pBody, BYTE *lpBuf) {
	...
	for (BYTE i = 0; i < 3; i++) {
		nPos += CFA_DI_LOSTV_TOTAL(&pFA->DT_STR_PS_ERR_PHASE[i], (BYTE*) &lpBuf[nPos]);
	}
	nPos += CFA_DI_TIMEYS(&pFA->DT_END_PS_ERR_TIME, (BYTE*) &lpBuf[nPos]);
	nPos += CFA_DI_LOSTV_TOTAL(&pFA->DT_END_PS_ERR_TOTAL, (BYTE*) &lpBuf[nPos]);
	for (BYTE i = 0; i < 3; i++) {
		nPos += CFA_DI_LOSTV_TOTAL(&pFA->DT_END_PS_ERR_PHASE[i], (BYTE*) &lpBuf[nPos]);
	}
	//
	return nPos;
}
...
static int CFP_DI180XFF0X(BYTE *lpBuf, void *pSrc) {
	...
	for (BYTE i = 0; i < 3; i++) {
		nPos += CFP_DI_START_LOSTV_PHASE((BYTE*) &lpBuf[nPos], &pFA->DT_START_LI_PHASE[i]);
	}

	nPos += CFP_DI_TIMEYS((BYTE*) &lpBuf[nPos], &pFA->DT_END_LI_TIME);
	nPos += CFP_DI_LOSTV_TOTAL((BYTE*) &lpBuf[nPos], &pFA->DT_END_LI_TOTAL);
	for (BYTE i = 0; i < 3; i++) {
		nPos += CFP_DI_END_LOSTV_PHASE((BYTE*) &lpBuf[nPos], &pFA->DT_START_LI_PHASE[i]);
	}
...
static int CFA_DI180XFF0X(void *pBody, BYTE *lpBuf) {
	...
	for (BYTE i = 0; i < 3; i++) {
		nPos += CFA_DI_START_LOSTV_PHASE(&pFA->DT_START_LI_PHASE[i], (BYTE*) &lpBuf[nPos]);
	}

	nPos += CFA_DI_TIMEYS(&pFA->DT_END_LI_TIME, (BYTE*) &lpBuf[nPos]);
	nPos += CFA_DI_LOSTV_TOTAL(&pFA->DT_END_LI_TOTAL, (BYTE*) &lpBuf[nPos]);
	for (BYTE i = 0; i < 3; i++) {
		nPos += CFA_DI_END_LOSTV_PHASE(&pFA->DT_START_LI_PHASE[i], (BYTE*) &lpBuf[nPos]);
	}
	//
	return nPos;
}
...
static int CFP_DI1F00FF0X(BYTE *lpBuf, void *pSrc) {
	...
	nPos += CFP_DI_LOSTV_TOTAL((BYTE*) &lpBuf[nPos], &pFA->DT_START_PF_DOWN_TOTAL);

	nPos += CFP_DI_TIMEYS((BYTE*) &lpBuf[nPos], &pFA->DT_END_PF_DOWN_TIME);
	nPos += CFP_DI_LOSTV_TOTAL((BYTE*) &lpBuf[nPos], &pFA->DT_END_PF_DOWN_TOTAL);
...
static int CFA_DI1F00FF0X(void *pBody, BYTE *lpBuf) {
	...
	nPos += CFA_DI_LOSTV_TOTAL(&pFA->DT_START_PF_DOWN_TOTAL, (BYTE*) &lpBuf[nPos]);

	nPos += CFA_DI_TIMEYS(&pFA->DT_END_PF_DOWN_TIME, (BYTE*) &lpBuf[nPos]);
	nPos += CFA_DI_LOSTV_TOTAL(&pFA->DT_END_PF_DOWN_TOTAL, (BYTE*) &lpBuf[nPos]);
...
//电量
		{ 0x00000000, 0x00003F0C, 0x01, &CFP_ENERGY, &CFA_ENERGY, &CPrint_ENERGY }, { 0x0000FF00, 0x0000FF0C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x00010000, 0x00013F0C, 0x01,
				&CFP_ENERGY, &CFA_ENERGY, &CPrint_ENERGY }, { 0x0001FF00, 0x0001FF0C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x00020000, 0x00023F0C, 0x01, &CFP_ENERGY, &CFA_ENERGY,
				&CPrint_ENERGY }, { 0x0002FF00, 0x0002FF0C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x00030000, 0x00033F0C, 0x01, &CFP_ENERGY, &CFA_ENERGY, &CPrint_ENERGY }, {
				0x0003FF00, 0x0003FF0C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x00040000, 0x00043F0C, 0x01, &CFP_ENERGY, &CFA_ENERGY, &CPrint_ENERGY }, { 0x0004FF00, 0x0004FF0C,
				0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x00050000, 0x00053F0C, 0x01, &CFP_ENERGY, &CFA_ENERGY, &CPrint_ENERGY }, { 0x0005FF00, 0x0005FF0C, 0x01, &CFP_DI000XFF0X,
				&CFA_DI000XFF0X, &CPrint_NONE }, { 0x00060000, 0x00063F0C, 0x01, &CFP_ENERGY, &CFA_ENERGY, &CPrint_ENERGY }, { 0x0006FF00, 0x0006FF0C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X,
				&CPrint_NONE }, { 0x00070000, 0x00073F0C, 0x01, &CFP_ENERGY, &CFA_ENERGY, &CPrint_ENERGY }, { 0x0007FF00, 0x0007FF0C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, {
				0x00080000, 0x00083F0C, 0x01, &CFP_ENERGY, &CFA_ENERGY, &CPrint_ENERGY }, { 0x0008FF00, 0x0008FF0C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x00090000, 0x00093F0C,
				0x01, &CFP_ENERGY, &CFA_ENERGY, &CPrint_ENERGY }, { 0x0009FF00, 0x0009FF0C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x000A0000, 0x000A3F0C, 0x01, &CFP_ENERGY,
				&CFA_ENERGY, &CPrint_ENERGY }, { 0x000AFF00, 0x000AFF0C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE },

		{ 0x000B0000, 0x000B000C, 0x01, &CFP_ENERGY, &CFA_ENERGY, &CPrint_ENERGY }, { 0x00150000, 0x001E000C, 0x01, &CFP_ENERGY, &CFA_ENERGY, &CPrint_ENERGY }, { 0x00290000, 0x0032000C, 0x01,
				&CFP_ENERGY, &CFA_ENERGY, &CPrint_ENERGY }, { 0x003D0000, 0x0046000C, 0x01, &CFP_ENERGY, &CFA_ENERGY, &CPrint_ENERGY },

		{ 0x00800000, 0x0086000C, 0x01, &CFP_ENERGY, &CFA_ENERGY, &CPrint_ENERGY }, { 0x00940000, 0x009A000C, 0x01, &CFP_ENERGY, &CFA_ENERGY, &CPrint_ENERGY }, { 0x00A80000, 0x00AE000C, 0x01,
				&CFP_ENERGY, &CFA_ENERGY, &CPrint_ENERGY }, { 0x00BC0000, 0x00C2000C, 0x01, &CFP_ENERGY, &CFA_ENERGY, &CPrint_ENERGY },

		//需量
		{ 0x01000000, 0x01003F0C, 0x01, &CFP_XLVAL_TIME, &CFA_XLVAL_TIME, &CPrint_XLVAL_TIME }, { 0x0100FF00, 0x0100FF0C, 0x01, &CFP_DI010XFF0X, &CFA_DI010XFF0X, &CPrint_NONE }, { 0x01010000,
				0x01013F0C, 0x01, &CFP_XLVAL_TIME, &CFA_XLVAL_TIME, &CPrint_XLVAL_TIME }, { 0x0101FF00, 0x0101FF0C, 0x01, &CFP_DI010XFF0X, &CFA_DI010XFF0X, &CPrint_NONE }, { 0x01020000, 0x01023F0C,
				0x01, &CFP_XLVAL_TIME, &CFA_XLVAL_TIME, &CPrint_XLVAL_TIME }, { 0x0102FF00, 0x0102FF0C, 0x01, &CFP_DI010XFF0X, &CFA_DI010XFF0X, &CPrint_NONE }, { 0x01030000, 0x01033F0C, 0x01,
				&CFP_XLVAL_TIME, &CFA_XLVAL_TIME, &CPrint_XLVAL_TIME }, { 0x0103FF00, 0x0103FF0C, 0x01, &CFP_DI010XFF0X, &CFA_DI010XFF0X, &CPrint_NONE }, { 0x01040000, 0x01043F0C, 0x01,
				&CFP_XLVAL_TIME, &CFA_XLVAL_TIME, &CPrint_XLVAL_TIME }, { 0x0104FF00, 0x0104FF0C, 0x01, &CFP_DI010XFF0X, &CFA_DI010XFF0X, &CPrint_NONE }, { 0x01050000, 0x01053F0C, 0x01,
				&CFP_XLVAL_TIME, &CFA_XLVAL_TIME, &CPrint_XLVAL_TIME }, { 0x0105FF00, 0x0105FF0C, 0x01, &CFP_DI010XFF0X, &CFA_DI010XFF0X, &CPrint_NONE }, { 0x01060000, 0x01063F0C, 0x01,
				&CFP_XLVAL_TIME, &CFA_XLVAL_TIME, &CPrint_XLVAL_TIME }, { 0x0106FF00, 0x0106FF0C, 0x01, &CFP_DI010XFF0X, &CFA_DI010XFF0X, &CPrint_NONE }, { 0x01070000, 0x01073F0C, 0x01,
				&CFP_XLVAL_TIME, &CFA_XLVAL_TIME, &CPrint_XLVAL_TIME }, { 0x0107FF00, 0x0107FF0C, 0x01, &CFP_DI010XFF0X, &CFA_DI010XFF0X, &CPrint_NONE }, { 0x01080000, 0x01083F0C, 0x01,
				&CFP_XLVAL_TIME, &CFA_XLVAL_TIME, &CPrint_XLVAL_TIME }, { 0x0108FF00, 0x0108FF0C, 0x01, &CFP_DI010XFF0X, &CFA_DI010XFF0X, &CPrint_NONE }, { 0x01090000, 0x01093F0C, 0x01,
				&CFP_XLVAL_TIME, &CFA_XLVAL_TIME, &CPrint_XLVAL_TIME }, { 0x0109FF00, 0x0109FF0C, 0x01, &CFP_DI010XFF0X, &CFA_DI010XFF0X, &CPrint_NONE }, { 0x010A0000, 0x010A3F0C, 0x01,
				&CFP_XLVAL_TIME, &CFA_XLVAL_TIME, &CPrint_XLVAL_TIME }, { 0x010AFF00, 0x010AFF0C, 0x01, &CFP_DI010XFF0X, &CFA_DI010XFF0X, &CPrint_NONE }, { 0x01150000, 0x011E000C, 0x01,
				&CFP_XLVAL_TIME, &CFA_XLVAL_TIME, &CPrint_XLVAL_TIME }, { 0x01290000, 0x0132000C, 0x01, &CFP_XLVAL_TIME, &CFA_XLVAL_TIME, &CPrint_XLVAL_TIME }, { 0x013D0000, 0x0146000C, 0x01,
				&CFP_XLVAL_TIME, &CFA_XLVAL_TIME, &CPrint_XLVAL_TIME }, { 0x01940000, 0x019A000C, 0x01, &CFP_XLVAL_TIME, &CFA_XLVAL_TIME, &CPrint_XLVAL_TIME }, { 0x01A80000, 0x01AE000C, 0x01,
				&CFP_XLVAL_TIME, &CFA_XLVAL_TIME, &CPrint_XLVAL_TIME }, { 0x01BC0000, 0x01C2000C, 0x01, &CFP_XLVAL_TIME, &CFA_XLVAL_TIME, &CPrint_XLVAL_TIME },
		//变量
		{ 0x02010100, 0x02010300, 0x01, &CFP_WORD, &CFA_WORD, &CPrint_WORD }, { 0x0201FF00, 0x0201FF00, 0x01, &CFP_DI0201FF00, &CFA_DI0201FF00, &CPrint_NONE }, { 0x02020100, 0x02020300, 0x01,
				&CFP_CURRENT, &CFA_CURRENT, &CPrint_CURRENT }, { 0x0202FF00, 0x0202FF00, 0x01, &CFP_DI0202FF00, &CFA_DI0202FF00, &CPrint_NONE }, { 0x02030000, 0x02030300, 0x01, &CFP_POWER, &CFA_POWER,
				&CPrint_POWER }, { 0x0203FF00, 0x0203FF00, 0x01, &CFP_DI0203FF00, &CFA_DI0203FF00, &CPrint_NONE }, { 0x02040000, 0x02040300, 0x01, &CFP_POWER, &CFA_POWER, &CPrint_POWER }, {
				0x0204FF00, 0x0204FF00, 0x01, &CFP_DI0203FF00, &CFA_DI0203FF00, &CPrint_NONE }, { 0x02050000, 0x02050300, 0x01, &CFP_POWER, &CFA_POWER, &CPrint_POWER }, { 0x0205FF00, 0x0205FF00, 0x01,
				&CFP_DI0203FF00, &CFA_DI0203FF00, &CPrint_NONE }, { 0x02060000, 0x02060300, 0x01, &CFP_PF, &CFA_PF, &CPrint_PF }, { 0x0206FF00, 0x0206FF00, 0x01, &CFP_DI0206FF00, &CFA_DI0206FF00,
				&CPrint_NONE }, { 0x02070100, 0x02070300, 0x01, &CFP_PANGLE, &CFA_PANGLE, &CPrint_PANGLE }, { 0x0207FF00, 0x0207FF00, 0x01, &CFP_DI0207FF00, &CFA_DI0207FF00, &CPrint_NONE },

		//谐波含量
		{ 0x020A0101, 0x020A0115, 0x01, &CFP_HRRATE, &CFA_HRRATE, &CPrint_HRRATE }, { 0x020A01FF, 0x020A01FF, 0x01, &CFP_DI020A0XFF, &CFA_DI020A0XFF, &CPrint_NONE }, { 0x020A0201, 0x020A0215, 0x01,
				&CFP_HRRATE, &CFA_HRRATE, &CPrint_HRRATE }, { 0x020A02FF, 0x020A02FF, 0x01, &CFP_DI020A0XFF, &CFA_DI020A0XFF, &CPrint_NONE }, { 0x020A0301, 0x020A0315, 0x01, &CFP_HRRATE, &CFA_HRRATE,
				&CPrint_HRRATE }, { 0x020A03FF, 0x020A03FF, 0x01, &CFP_DI020A0XFF, &CFA_DI020A0XFF, &CPrint_NONE },

		{ 0x020B0101, 0x020B0115, 0x01, &CFP_HRRATE, &CFA_HRRATE, &CPrint_HRRATE }, { 0x020B01FF, 0x020B01FF, 0x01, &CFP_DI020A0XFF, &CFA_DI020A0XFF, &CPrint_NONE }, { 0x020B0201, 0x020B0215, 0x01,
				&CFP_HRRATE, &CFA_HRRATE, &CPrint_HRRATE }, { 0x020B02FF, 0x020B02FF, 0x01, &CFP_DI020A0XFF, &CFA_DI020A0XFF, &CPrint_NONE }, { 0x020B0301, 0x020B0315, 0x01, &CFP_HRRATE, &CFA_HRRATE,
				&CPrint_HRRATE }, { 0x020B03FF, 0x020B03FF, 0x01, &CFP_DI020A0XFF, &CFA_DI020A0XFF, &CPrint_NONE },
		//
		//0280000X
		{ 0x02800001, 0x02800001, 0x01, &CFP_CURRENT, &CFA_CURRENT, &CPrint_CURRENT }, { 0x02800002, 0x02800002, 0x01, &CFP_DOUBLE_22, &CFA_DOUBLE_22, &CPrint_DOUBLE_22 }, { 0x02800003, 0x02800006,
				0x01, &CFP_POWER, &CFA_POWER, &CPrint_POWER }, { 0x02800007, 0x02800007, 0x01, &CFP_DOUBLE_31, &CFA_DOUBLE_31, &CPrint_DOUBLE_31 }, { 0x02800008, 0x02800009, 0x01, &CFP_VOLTAGE,
				&CFA_VOLTAGE, &CPrint_VOLTAGE }, { 0x0280000A, 0x0280000A, 0x01, &CFP_DWORD, &CFA_DWORD, &CPrint_DWORD }, { 0x0280000B, 0x0280000B, 0x01, &CFP_DOUBLE_62, &CFA_DOUBLE_62,
				&CPrint_DOUBLE_31 },

		{ 0x028000FF, 0x028000FF, 0x01, &CFP_DI028000FF, &CFA_DI028000FF, &CPrint_NONE },

		//全失压记录
		{ 0x03050000, 0x03050000, 0x01, &CFP_DI03050000, &CFA_DI03050000, &CPrint_LONG3BYTE }, { 0x03050001, 0x0305000A, 0x01, &CFP_DI03050001, &CFA_DI03050001, &CPrint_NONE },

		//辅助电源失电记录
		{ 0x03060000, 0x03060000, 0x01, &CFP_DI03050000, &CFA_DI03050000, &CPrint_LONG3BYTE }, { 0x03060001, 0x0306000A, 0x01, &CFP_DI03060001, &CFA_DI03060001, &CPrint_NONE },

		//掉电记录
		{ 0x03110000, 0x03110000, 0x01, &CFP_LONG3BYTE, &CFA_DI03050000, &CPrint_LONG3BYTE }, { 0x03110001, 0x0311000A, 0x01, &CFP_DI03110001, &CFA_DI03110001, &CPrint_NONE },

		//编程记录
		{ 0x03300000, 0x03300000, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x03300001, 0x0330000A, 0x01, &CFP_DI03300001, &CFA_DI03300001, &CPrint_NONE },

		//电表清0记录
		{ 0x03300100, 0x03300100, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x03300101, 0x0330010A, 0x01, &CFP_DI03300101, &CFA_DI03300101, &CPrint_NONE },

		//需量清0记录
		{ 0x03300200, 0x03300200, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x03300201, 0x0330020A, 0x01, &CFP_DI03300201, &CFA_DI03300201, &CPrint_NONE },

		//事件清0记录
		{ 0x03300300, 0x03300300, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x03300301, 0x0330030A, 0x01, &CFP_DI03300301, &CFA_DI03300301, &CPrint_NONE },

		//校时记录
		{ 0x03300400, 0x03300400, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x03300401, 0x0330040A, 0x01, &CFP_DI03300401, &CFA_DI03300401, &CPrint_NONE },

		//时段表编程记录
		{ 0x03300500, 0x03300500, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x03300501, 0x0330050A, 0x01, &CFP_DI03300501, &CFA_DI03300501, &CPrint_NONE },

		//时区表编程记录
		{ 0x03300600, 0x03300600, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x03300601, 0x0330060A, 0x01, &CFP_DI03300601, &CFA_DI03300601, &CPrint_NONE },

		//周休日编程记录
		{ 0x03300700, 0x03300700, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x03300701, 0x0330070A, 0x01, &CFP_DI03300701, &CFA_DI03300701, &CPrint_NONE },

		//节假日编程记录
		{ 0x03300800, 0x03300800, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x03300801, 0x0330080A, 0x01, &CFP_DI03300801, &CFA_DI03300801, &CPrint_NONE },

		//有功组合方式编程记录
		{ 0x03300900, 0x03300900, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x03300901, 0x0330090A, 0x01, &CFP_DI03300901, &CFA_DI03300901, &CPrint_NONE },

		//无功组合方式1编程记录
		{ 0x03300A00, 0x03300A00, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x03300A01, 0x03300A0A, 0x01, &CFP_DI03300901, &CFA_DI03300901, &CPrint_NONE },

		//无功组合方式2编程记录
		{ 0x03300B00, 0x03300B00, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x03300B01, 0x03300B0A, 0x01, &CFP_DI03300901, &CFA_DI03300901, &CPrint_NONE },

		//结算日编程记录
		{ 0x03300C00, 0x03300C00, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x03300C01, 0x03300C0A, 0x01, &CFP_DI03300C01, &CFA_DI03300C01, &CPrint_NONE },

		//开表盖记录
		{ 0x03300D00, 0x03300D00, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x03300D01, 0x03300D0A, 0x01, &CFP_DI03300D01, &CFA_DI03300D01, &CPrint_NONE },

		//开端钮盒记录
		{ 0x03300E00, 0x03300E00, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x03300E01, 0x03300E0A, 0x01, &CFP_DI03300D01, &CFA_DI03300D01, &CPrint_NONE },

//参数

		{ 0x04000101, 0x04000101, 0x01, &CFP_DI04000101, &CFA_DI04000101, &CPrint_DI04000101 }, { 0x04000102, 0x04000102, 0x01, &CFP_DI04000102, &CFA_DI04000102, &CPrint_DI04000102 }, { 0x04000103,
				0x04000103, 0x01, &CFP_BYTE, &CFA_BYTE, &CPrint_BYTE }, { 0x04000104, 0x04000104, 0x01, &CFP_BYTE, &CFA_BYTE, &CPrint_BYTE }, { 0x04000105, 0x04000105, 0x01, &CFP_WORD, &CFA_WORD,
				&CPrint_WORD }, { 0x04000106, 0x04000109, 0x01, &CFP_DI_TIME, &CFA_DI_TIME, &CPrint_DI_TIME }, { 0x040001FF, 0x040001FF, 0x01, &CFP_DI040001FF, &CFA_DI040001FF, &CPrint_NONE },

		{ 0x04000201, 0x04000204, 0x01, &CFP_BYTE, &CFA_BYTE, &CPrint_BYTE }, { 0x04000205, 0x04000205, 0x01, &CFP_WORD, &CFA_WORD, &CPrint_WORD }, { 0x04000206, 0x04000207, 0x01, &CFP_BYTE,
				&CFA_BYTE, &CPrint_BYTE }, { 0x040002FF, 0x040002FF, 0x01, &CFP_DI040002FF, &CFA_DI040002FF, &CPrint_NONE },

		{ 0x04000301, 0x04000305, 0x01, &CFP_BYTE, &CFA_BYTE, &CPrint_BYTE }, { 0x04000306, 0x04000307, 0x01, &CFP_DWORD, &CFA_DWORD, &CPrint_DWORD }, { 0x040003FF, 0x040003FF, 0x01, &CFP_DI040003FF,
				&CFA_DI040003FF, &CPrint_NONE },

		{ 0x04000401, 0x04000401, 0x01, &CFP_DI04000401, &CFA_DI04000401, &CPrint_NONE }, { 0x04000402, 0x04000402, 0x01, &CFP_DI04000402, &CFA_DI04000402, &CPrint_NONE }, { 0x04000403, 0x04000403,
				0x01, &CFP_DI04000403, &CFA_DI04000403, &CPrint_NONE }, { 0x04000404, 0x04000404, 0x01, &CFP_DI04000404, &CFA_DI04000404, &CPrint_NONE }, { 0x04000405, 0x04000405, 0x01,
				&CFP_DI04000405, &CFA_DI04000405, &CPrint_NONE }, { 0x04000406, 0x04000406, 0x01, &CFP_DI04000406, &CFA_DI04000406, &CPrint_NONE }, { 0x04000407, 0x04000407, 0x01, &CFP_DI04000407,
				&CFA_DI04000407, &CPrint_NONE }, { 0x04000408, 0x04000408, 0x01, &CFP_DI04000408, &CFA_DI04000408, &CPrint_NONE }, { 0x04000409, 0x04000409, 0x01, &CFP_DI04000409, &CFA_DI04000409,
				&CPrint_NONE }, { 0x0400040A, 0x0400040A, 0x01, &CFP_DI0400040A, &CFA_DI0400040A, &CPrint_NONE }, { 0x0400040B, 0x0400040B, 0x01, &CFP_DI0400040B, &CFA_DI0400040B, &CPrint_NONE }, {
				0x0400040C, 0x0400040C, 0x01, &CFP_DI0400040C, &CFA_DI0400040C, &CPrint_NONE }, { 0x0400040D, 0x0400040D, 0x01, &CFP_DI0400040D, &CFA_DI0400040D, &CPrint_NONE }, { 0x0400040E,
				0x0400040E, 0x01, &CFP_DI0400040E, &CFA_DI0400040E, &CPrint_NONE }, { 0x040004FF, 0x040004FF, 0x01, &CFP_DI040004FF, &CFA_DI040004FF, &CPrint_NONE },

		//电表状态字
		{ 0x04000501, 0x04000507, 0x01, nullptr, nullptr, &CPrint_WORD }, { 0x040005FF, 0x040005FF, 0x01, nullptr, nullptr, &CPrint_NONE },

		//有无功组合方式
		{ 0x04000601, 0x04000603, 0x01, nullptr, nullptr, &CPrint_NONE }, { 0x040006FF, 0x040006FF, 0x01, nullptr, nullptr, &CPrint_NONE },

		//串口通讯特征字
		{ 0x04000701, 0x04000705, 0x01, nullptr, nullptr, &CPrint_NONE }, { 0x040007FF, 0x040007FF, 0x01, nullptr, nullptr, &CPrint_NONE },

		//周休日
		{ 0x04000801, 0x04000801, 0x01, &CFP_DI04000801, &CFA_DI04000801, &CPrint_NONE }, { 0x04000802, 0x04000802, 0x01, &CFP_BYTE, &CFA_BYTE, &CPrint_NONE }, { 0x040008FF, 0x040008FF, 0x01,
				&CFP_DI040008FF, &CFA_DI040008FF, &CPrint_NONE },

		//冻结模式字
		{ 0x04000901, 0x04000906, 0x01, nullptr, nullptr, &CPrint_NONE }, { 0x040009FF, 0x040009FF, 0x01, nullptr, nullptr, &CPrint_NONE },

		{ 0x04000A01, 0x04000A01, 0x01, &CFP_DI_TIME, &CFA_DI_TIME, &CPrint_NONE }, { 0x04000A02, 0x04000A07, 0x01, &CFP_WORD, &CFA_WORD, &CPrint_NONE }, { 0x04000AFF, 0x04000AFF, 0x01,
				&CFP_DI04000AFF, &CFA_DI04000AFF, &CPrint_NONE },

		{ 0x04000B01, 0x04000B03, 0x01, &CFP_DI04000B0X, &CFA_DI04000B0X, &CPrint_NONE }, { 0x04000BFF, 0x04000BFF, 0x01, &CFP_DI04000BFF, &CFA_DI04000BFF, &CPrint_NONE },

		{ 0x04000C01, 0x04000C0A, 0x01, &CFP_DI04000C0X, &CFA_DI04000C0X, &CPrint_NONE }, { 0x04000CFF, 0x04000CFF, 0x01, &CFP_DI04000CFF, &CFA_DI04000CFF, &CPrint_NONE },

		//功率及电压限值
		{ 0x04000E01, 0x04000E02, 0x01, &CFP_DOUBLE_24, &CFA_DOUBLE_24, &CPrint_NONE }, { 0x04000E03, 0x04000E04, 0x01, &CFP_WORD, &CFA_WORD, &CPrint_NONE }, { 0x04000EFF, 0x04000EFF, 0x01,
				&CFP_DI04000EFF, &CFA_DI04000EFF, &CPrint_NONE },

		//电量限值
		{ 0x04000F01, 0x04000F04, 0x01, &CFP_DOUBLE_62, &CFA_DOUBLE_62, &CPrint_NONE }, { 0x04000FFF, 0x04000FFF, 0x01, &CFP_DI04000FFF, &CFA_DI04000FFF, &CPrint_NONE },

		//
		{ 0x04001001, 0x04001001, 0x01, &CFP_DOUBLE_62, &CFA_DOUBLE_62, &CPrint_NONE }, { 0x04001002, 0x04001002, 0x01, &CFP_DOUBLE_62, &CFA_DOUBLE_62, &CPrint_NONE }, { 0x04001003, 0x04001003, 0x01,
				&CFP_DOUBLE_62, &CFA_DOUBLE_62, &CPrint_NONE }, { 0x04001004, 0x04001004, 0x01, &CFP_DOUBLE_62, &CFA_DOUBLE_62, &CPrint_NONE }, { 0x04001004, 0x04001005, 0x01, &CFP_DOUBLE_62,
				&CFA_DOUBLE_62, &CPrint_NONE }, { 0x040010FF, 0x040010FF, 0x01, &CFP_DI040010FF, &CFA_DI040010FF, &CPrint_NONE },

		{ 0x04001101, 0x04001101, 0x01, &CFP_DI04000801, &CFA_DI04000801, &CPrint_NONE }, { 0x040011FF, 0x040011FF, 0x01, &CFP_DI040011FF, &CFA_DI040011FF, &CPrint_NONE },

		//整点冻结参数
		{ 0x04001201, 0x04001201, 0x01, &CFP_DI_TIME, &CFA_DI_TIME, &CPrint_NONE }, { 0x04001202, 0x04001202, 0x01, &CFP_BYTE, &CFA_BYTE, &CPrint_NONE }, { 0x04001203, 0x04001203, 0x01,
				&CFP_DI04001203, &CFA_DI04001203, &CPrint_NONE }, { 0x040012FF, 0x040012FF, 0x01, &CFP_DI040012FF, &CFA_DI040012FF, &CPrint_NONE },

		//无线通信参数
		{ 0x04001301, 0x04001301, 0x01, &CFP_BYTE, &CFA_BYTE, &CPrint_NONE }, { 0x040013FF, 0x040013FF, 0x01, &CFP_BYTE, &CFA_BYTE, &CPrint_NONE },

		//跳闸延时时间
		{ 0x04001401, 0x04001401, 0x01, &CFP_WORD, &CFA_WORD, &CPrint_NONE }, { 0x040014FF, 0x040014FF, 0x01, &CFP_WORD, &CFA_WORD, &CPrint_NONE },

		{ 0x04010000, 0x04010000, 0x01, &CFP_DI04010000, &CFA_DI04010000, &CPrint_NONE }, { 0x04010001, 0x04010008, 0x01, &CFP_DI04010001, &CFA_DI04010001, &CPrint_NONE }, { 0x04020000, 0x04020000,
				0x01, &CFP_DI04010000, &CFA_DI04010000, &CPrint_NONE }, { 0x04020001, 0x04020008, 0x01, &CFP_DI04010001, &CFA_DI04010001, &CPrint_NONE },

		//第一套梯值
		{ 0x04060001, 0x0406000E, 0x01, &CFP_DOUBLE_62, &CFA_DOUBLE_62, &CPrint_NONE }, { 0x040600FF, 0x040600FF, 0x01, &CFP_DI040600FF, &CFA_DI040600FF, &CPrint_NONE },

		//第一套电价
		{ 0x04060101, 0x0406010E, 0x01, &CFP_DOUBLE_44, &CFA_DOUBLE_44, &CPrint_NONE }, { 0x040601FF, 0x040601FF, 0x01, &CFP_DI040601FF, &CFA_DI040601FF, &CPrint_NONE },

		//第二套梯值
		{ 0x04060201, 0x0406020E, 0x01, &CFP_DOUBLE_62, &CFA_DOUBLE_62, &CPrint_NONE }, { 0x040602FF, 0x040602FF, 0x01, &CFP_DI040602FF, &CFA_DI040602FF, &CPrint_NONE },

		//第二套电价
		{ 0x04060301, 0x0406030E, 0x01, &CFP_DOUBLE_44, &CFA_DOUBLE_44, &CPrint_NONE }, { 0x040603FF, 0x040603FF, 0x01, &CFP_DI040603FF, &CFA_DI040603FF, &CPrint_NONE },

		//
		{ 0x04090101, 0x04090101, 0x01, &CFP_WORD, &CFA_WORD, &CPrint_NONE }, { 0x04090102, 0x04090102, 0x01, &CFP_WORD, &CFA_WORD, &CPrint_NONE }, { 0x04090103, 0x04090103, 0x01, &CFP_DOUBLE_24,
				&CFA_DOUBLE_24, &CPrint_NONE }, { 0x04090104, 0x04090104, 0x01, &CFP_BYTE, &CFA_BYTE, &CPrint_NONE }, { 0x040901FF, 0x040901FF, 0x01, &CFP_DI040901FF, &CFA_DI040901FF, &CPrint_NONE },

		{ 0x04090201, 0x04090201, 0x01, &CFP_WORD, &CFA_WORD, &CPrint_NONE }, { 0x04090202, 0x04090202, 0x01, &CFP_BYTE, &CFA_BYTE, &CPrint_NONE }, { 0x040902FF, 0x040902FF, 0x01, &CFP_DI040902FF,
				&CFA_DI040902FF, &CPrint_NONE },

		{ 0x04090301, 0x04090301, 0x01, &CFP_WORD, &CFA_WORD, &CPrint_NONE }, { 0x04090302, 0x04090302, 0x01, &CFP_BYTE, &CFA_BYTE, &CPrint_NONE }, { 0x040903FF, 0x040903FF, 0x01, &CFP_DI040903FF,
				&CFA_DI040903FF, &CPrint_NONE },

		{ 0x04090401, 0x04090401, 0x01, &CFP_WORD, &CFA_WORD, &CPrint_NONE }, { 0x04090402, 0x04090402, 0x01, &CFP_DOUBLE_24, &CFA_DOUBLE_24, &CPrint_NONE }, { 0x04090403, 0x04090403, 0x01, &CFP_BYTE,
				&CFA_BYTE, &CPrint_NONE }, { 0x040904FF, 0x040904FF, 0x01, &CFP_DI040904FF, &CFA_DI040904FF, &CPrint_NONE },

		{ 0x04090501, 0x04090501, 0x01, &CFP_DOUBLE_22, &CFA_DOUBLE_22, &CPrint_NONE }, { 0x04090502, 0x04090502, 0x01, &CFP_BYTE, &CFA_BYTE, &CPrint_NONE }, { 0x040905FF, 0x040905FF, 0x01,
				&CFP_DI040905FF, &CFA_DI040905FF, &CPrint_NONE },

		{ 0x04090601, 0x04090601, 0x01, &CFP_DOUBLE_22, &CFA_DOUBLE_22, &CPrint_NONE }, { 0x04090602, 0x04090602, 0x01, &CFP_BYTE, &CFA_BYTE, &CPrint_NONE }, { 0x040906FF, 0x040906FF, 0x01,
				&CFP_DI040906FF, &CFA_DI040906FF, &CPrint_NONE },

		{ 0x04090701, 0x04090701, 0x01, &CFP_WORD, &CFA_WORD, &CPrint_NONE }, { 0x04090702, 0x04090702, 0x01, &CFP_DOUBLE_24, &CFA_DOUBLE_24, &CPrint_NONE }, { 0x04090703, 0x04090703, 0x01,
				&CFP_DOUBLE_24, &CFA_DOUBLE_24, &CPrint_NONE }, { 0x04090704, 0x04090704, 0x01, &CFP_BYTE, &CFA_BYTE, &CPrint_NONE }, { 0x040907FF, 0x040907FF, 0x01, &CFP_DI040907FF, &CFA_DI040907FF,
				&CPrint_NONE },

		{ 0x04090801, 0x04090801, 0x01, &CFP_DOUBLE_31, &CFA_DOUBLE_31, &CPrint_NONE }, { 0x04090802, 0x04090802, 0x01, &CFP_BYTE, &CFA_BYTE, &CPrint_NONE }, { 0x040908FF, 0x040908FF, 0x01,
				&CFP_DI040908FF, &CFA_DI040908FF, &CPrint_NONE },

		{ 0x04090901, 0x04090901, 0x01, &CFP_WORD, &CFA_WORD, &CPrint_NONE }, { 0x04090902, 0x04090902, 0x01, &CFP_DOUBLE_24, &CFA_DOUBLE_24, &CPrint_NONE }, { 0x04090903, 0x04090903, 0x01, &CFP_BYTE,
				&CFA_BYTE, &CPrint_NONE }, { 0x040909FF, 0x040909FF, 0x01, &CFP_DI040909FF, &CFA_DI040909FF, &CPrint_NONE },

		{ 0x04090A01, 0x04090A01, 0x01, &CFP_DOUBLE_24, &CFA_DOUBLE_24, &CPrint_NONE }, { 0x04090A02, 0x04090A02, 0x01, &CFP_BYTE, &CFA_BYTE, &CPrint_NONE }, { 0x04090AFF, 0x04090AFF, 0x01,
				&CFP_DI04090AFF, &CFA_DI04090AFF, &CPrint_NONE },

		{ 0x04090B01, 0x04090B01, 0x01, &CFP_DOUBLE_24, &CFA_DOUBLE_24, &CPrint_NONE }, { 0x04090B02, 0x04090B02, 0x01, &CFP_BYTE, &CFA_BYTE, &CPrint_NONE }, { 0x04090BFF, 0x04090BFF, 0x01,
				&CFP_DI04090BFF, &CFA_DI04090BFF, &CPrint_NONE },

		{ 0x04090C01, 0x04090C01, 0x01, &CFP_WORD, &CFA_WORD, &CPrint_NONE }, { 0x04090C02, 0x04090C02, 0x01, &CFP_WORD, &CFA_WORD, &CPrint_NONE }, { 0x04090CFF, 0x04090CFF, 0x01, &CFP_DI04090CFF,
				&CFA_DI04090CFF, &CPrint_NONE },

		{ 0x04090D01, 0x04090D01, 0x01, &CFP_DOUBLE_24, &CFA_DOUBLE_24, &CPrint_NONE }, { 0x04090D02, 0x04090D02, 0x01, &CFP_DOUBLE_24, &CFA_DOUBLE_24, &CPrint_NONE }, { 0x04090D03, 0x04090D03, 0x01,
				&CFP_BYTE, &CFA_BYTE, &CPrint_NONE }, { 0x04090DFF, 0x04090DFF, 0x01, &CFP_DI04090DFF, &CFA_DI04090DFF, &CPrint_NONE },

		{ 0x04090E01, 0x04090E01, 0x01, &CFP_DOUBLE_13, &CFA_DOUBLE_13, &CPrint_NONE }, { 0x04090E02, 0x04090E02, 0x01, &CFP_BYTE, &CFA_BYTE, &CPrint_NONE }, { 0x04090EFF, 0x04090EFF, 0x01,
				&CFP_DI04090EFF, &CFA_DI04090EFF, &CPrint_NONE },

		{ 0x04090F01, 0x04090F01, 0x01, &CFP_DOUBLE_22, &CFA_DOUBLE_22, &CPrint_NONE }, { 0x04090F02, 0x04090F02, 0x01, &CFP_BYTE, &CFA_BYTE, &CPrint_NONE }, { 0x04090FFF, 0x04090FFF, 0x01,
				&CFP_DI04090FFF, &CFA_DI04090FFF, &CPrint_NONE },

		//版本号
		{ 0x04800001, 0x04800003, 0x01, &CFP_DI0480000X, &CFA_DI0480000X, &CPrint_NONE }, { 0x048000FF, 0x048000FF, 0x01, &CFP_DI048000FF, &CFA_DI048000FF, &CPrint_NONE },

		{ 0x04820000, 0x04820000, 0x01, &CFP_DI04820000, &CFA_DI04820000, &CPrint_NONE },

		//冻结数据
		{ 0x05000001, 0x0500000C, 0x01, &CFP_DI050X000X, &CFA_DI050X000X, &CPrint_NONE }, { 0x05000101, 0x0500010C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05000201, 0x0500020C,
				0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05000301, 0x0500030C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05000401, 0x0500040C, 0x01,
				&CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05000501, 0x0500050C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05000601, 0x0500060C, 0x01, &CFP_DI000XFF0X,
				&CFA_DI000XFF0X, &CPrint_NONE }, { 0x05000701, 0x0500070C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05000801, 0x0500080C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X,
				&CPrint_NONE }, { 0x05000901, 0x0500090C, 0x01, &CFP_DI010XFF0X, &CFA_DI010XFF0X, &CPrint_NONE }, { 0x05000A01, 0x05000A0C, 0x01, &CFP_DI010XFF0X, &CFA_DI010XFF0X, &CPrint_NONE }, {
				0x05001001, 0x0500100C, 0x01, &CFP_DI0500100X, &CFA_DI0500100X, &CPrint_NONE },

		{ 0x05010001, 0x0501000C, 0x01, &CFP_DI050X000X, &CFA_DI050X000X, &CPrint_NONE }, { 0x05010101, 0x0501010C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05010201, 0x0501020C,
				0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05010301, 0x0501030C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05010401, 0x0501040C, 0x01,
				&CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05010501, 0x0501050C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05010601, 0x0501060C, 0x01, &CFP_DI000XFF0X,
				&CFA_DI000XFF0X, &CPrint_NONE }, { 0x05010701, 0x0501070C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05010801, 0x0501080C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X,
				&CPrint_NONE }, { 0x05010901, 0x0501090C, 0x01, &CFP_DI010XFF0X, &CFA_DI010XFF0X, &CPrint_NONE }, { 0x05010A01, 0x05010A0C, 0x01, &CFP_DI010XFF0X, &CFA_DI010XFF0X, &CPrint_NONE }, {
				0x05011001, 0x0501100C, 0x01, &CFP_DI0500100X, &CFA_DI0500100X, &CPrint_NONE },

		{ 0x05020001, 0x0502000C, 0x01, &CFP_DI050X000X, &CFA_DI050X000X, &CPrint_NONE }, { 0x05020101, 0x0502010C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05020201, 0x0502020C,
				0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05020301, 0x0502030C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05020401, 0x0502040C, 0x01,
				&CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05020501, 0x0502050C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05020601, 0x0502060C, 0x01, &CFP_DI000XFF0X,
				&CFA_DI000XFF0X, &CPrint_NONE }, { 0x05020701, 0x0502070C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05020801, 0x0502080C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X,
				&CPrint_NONE }, { 0x05020901, 0x0502090C, 0x01, &CFP_DI010XFF0X, &CFA_DI010XFF0X, &CPrint_NONE }, { 0x05020A01, 0x05020A0C, 0x01, &CFP_DI010XFF0X, &CFA_DI010XFF0X, &CPrint_NONE }, {
				0x05021001, 0x0502100C, 0x01, &CFP_DI0500100X, &CFA_DI0500100X, &CPrint_NONE },

		{ 0x05030001, 0x0503000C, 0x01, &CFP_DI050X000X, &CFA_DI050X000X, &CPrint_NONE }, { 0x05030101, 0x0503010C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05030201, 0x0503020C,
				0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05030301, 0x0503030C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05030401, 0x0503040C, 0x01,
				&CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05030501, 0x0503050C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05030601, 0x0503060C, 0x01, &CFP_DI000XFF0X,
				&CFA_DI000XFF0X, &CPrint_NONE }, { 0x05030701, 0x0503070C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05030801, 0x0503080C, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X,
				&CPrint_NONE }, { 0x05030901, 0x0503090C, 0x01, &CFP_DI010XFF0X, &CFA_DI010XFF0X, &CPrint_NONE }, { 0x05030A01, 0x05030A0C, 0x01, &CFP_DI010XFF0X, &CFA_DI010XFF0X, &CPrint_NONE }, {
				0x05031001, 0x0503100C, 0x01, &CFP_DI0500100X, &CFA_DI0500100X, &CPrint_NONE },

		{ 0x05060001, 0x0506001F, 0x01, &CFP_DI050X000X, &CFA_DI050X000X, &CPrint_NONE }, { 0x05060101, 0x0506011F, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05060201, 0x0506021F,
				0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05060301, 0x0506031F, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05060401, 0x0506041F, 0x01,
				&CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05060501, 0x0506051F, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05060601, 0x0506061F, 0x01, &CFP_DI000XFF0X,
				&CFA_DI000XFF0X, &CPrint_NONE }, { 0x05060701, 0x0506071F, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X, &CPrint_NONE }, { 0x05060801, 0x0506081F, 0x01, &CFP_DI000XFF0X, &CFA_DI000XFF0X,
				&CPrint_NONE }, { 0x05060901, 0x0506091F, 0x01, &CFP_DI010XFF0X, &CFA_DI010XFF0X, &CPrint_NONE }, { 0x05060A01, 0x05060A1F, 0x01, &CFP_DI010XFF0X, &CFA_DI010XFF0X, &CPrint_NONE }, {
				0x05061001, 0x0506101F, 0x01, &CFP_DI0500100X, &CFA_DI0500100X, &CPrint_NONE },
		//事件

		//失压
		{ 0x10000001, 0x10000002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x100000FF, 0x100000FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE },

		{ 0x10000101, 0x10000201, 0x01, &CFP_DI_TIMEYS, &CFA_DI_TIMEYS, &CPrint_NONE }, { 0x1000FF01, 0x1000FF01, 0x01, &CFP_DI1000FF01, &CFA_DI1000FF01, &CPrint_NONE },

		{ 0x10010001, 0x10010002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x100100FF, 0x100100FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE }, { 0x1001FF01, 0x1001FF0A,
				0x01, &CFP_DI100XFF0X, &CFA_DI100XFF0X, &CPrint_NONE },

		{ 0x10020001, 0x10020002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x100200FF, 0x100200FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE }, { 0x1002FF01, 0x1002FF0A,
				0x01, &CFP_DI100XFF0X, &CFA_DI100XFF0X, &CPrint_NONE },

		{ 0x10030001, 0x10030002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x100300FF, 0x100300FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE }, { 0x1003FF01, 0x1003FF0A,
				0x01, &CFP_DI100XFF0X, &CFA_DI100XFF0X, &CPrint_NONE },

		//欠压
		{ 0x11010001, 0x11010002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x110100FF, 0x110100FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE }, { 0x1101FF01, 0x1101FF0A,
				0x01, &CFP_DI100XFF0X, &CFA_DI100XFF0X, &CPrint_NONE },

		{ 0x11020001, 0x11020002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x110200FF, 0x110200FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE }, { 0x1102FF01, 0x1102FF0A,
				0x01, &CFP_DI100XFF0X, &CFA_DI100XFF0X, &CPrint_NONE },

		{ 0x11030001, 0x11030002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x110300FF, 0x110300FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE }, { 0x1103FF01, 0x1103FF0A,
				0x01, &CFP_DI100XFF0X, &CFA_DI100XFF0X, &CPrint_NONE },

		//过压
		{ 0x12010001, 0x12010002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x120100FF, 0x120100FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE }, { 0x1201FF01, 0x1201FF0A,
				0x01, &CFP_DI100XFF0X, &CFA_DI100XFF0X, &CPrint_NONE },

		{ 0x12020001, 0x12020002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x120200FF, 0x120200FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE }, { 0x1202FF01, 0x1202FF0A,
				0x01, &CFP_DI100XFF0X, &CFA_DI100XFF0X, &CPrint_NONE },

		{ 0x12030001, 0x12030002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x120300FF, 0x120300FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE }, { 0x1203FF01, 0x1203FF0A,
				0x01, &CFP_DI100XFF0X, &CFA_DI100XFF0X, &CPrint_NONE },

		//断相
		{ 0x13010001, 0x13010002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x130100FF, 0x130100FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE }, { 0x1301FF01, 0x1301FF0A,
				0x01, &CFP_DI100XFF0X, &CFA_DI100XFF0X, &CPrint_NONE },

		{ 0x13020001, 0x13020002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x130200FF, 0x130200FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE }, { 0x1302FF01, 0x1302FF0A,
				0x01, &CFP_DI100XFF0X, &CFA_DI100XFF0X, &CPrint_NONE },

		{ 0x13030001, 0x13030002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x130300FF, 0x130300FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE }, { 0x1303FF01, 0x1303FF0A,
				0x01, &CFP_DI100XFF0X, &CFA_DI100XFF0X, &CPrint_NONE },

		//电压逆相序
		{ 0x14000001, 0x14000002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x140000FF, 0x140000FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE }, { 0x1400FF01, 0x1400FF0A,
				0x01, &CFP_DI1400FF0X, &CFA_DI1400FF0X, &CPrint_NONE },

		//电流逆相序
		{ 0x15000001, 0x15000002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x150000FF, 0x150000FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE }, { 0x1500FF01, 0x1500FF0A,
				0x01, &CFP_DI1400FF0X, &CFA_DI1400FF0X, &CPrint_NONE },

		//失流
		{ 0x18010001, 0x18010002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x180100FF, 0x180100FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE }, { 0x1801FF01, 0x1801FF0A,
				0x01, &CFP_DI180XFF0X, &CFA_DI180XFF0X, &CPrint_NONE },

		{ 0x18020001, 0x18020002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x180200FF, 0x180200FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE }, { 0x1802FF01, 0x1802FF0A,
				0x01, &CFP_DI180XFF0X, &CFA_DI180XFF0X, &CPrint_NONE },

		{ 0x18030001, 0x18030002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x180300FF, 0x180300FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE }, { 0x1803FF01, 0x1803FF0A,
				0x01, &CFP_DI180XFF0X, &CFA_DI180XFF0X, &CPrint_NONE },

		//过流
		{ 0x19010001, 0x19010002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x190100FF, 0x190100FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE }, { 0x1901FF01, 0x1901FF0A,
				0x01, &CFP_DI180XFF0X, &CFA_DI180XFF0X, &CPrint_NONE },

		{ 0x19020001, 0x19020002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x190200FF, 0x190200FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE }, { 0x1902FF01, 0x1902FF0A,
				0x01, &CFP_DI180XFF0X, &CFA_DI180XFF0X, &CPrint_NONE },

		{ 0x19030001, 0x19030002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x190300FF, 0x190300FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE }, { 0x1903FF01, 0x1903FF0A,
				0x01, &CFP_DI180XFF0X, &CFA_DI180XFF0X, &CPrint_NONE },

		//断流
		{ 0x1A010001, 0x1A010002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x1A0100FF, 0x1A0100FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE }, { 0x1A01FF01, 0x1A01FF0A,
				0x01, &CFP_DI180XFF0X, &CFA_DI180XFF0X, &CPrint_NONE },

		{ 0x1A020001, 0x1A020002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x1A0200FF, 0x1A0200FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE }, { 0x1A02FF01, 0x1A02FF0A,
				0x01, &CFP_DI180XFF0X, &CFA_DI180XFF0X, &CPrint_NONE },

		{ 0x1A030001, 0x1A030002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x1A0300FF, 0x1A0300FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE }, { 0x1A03FF01, 0x1A03FF0A,
				0x01, &CFP_DI180XFF0X, &CFA_DI180XFF0X, &CPrint_NONE },

		//功率因数超下限
		{ 0x1F000001, 0x1F000002, 0x01, &CFP_LONG3BYTE, &CFA_LONG3BYTE, &CPrint_LONG3BYTE }, { 0x1F0000FF, 0x1F0000FF, 0x01, &CFP_DI100000FF, &CFA_DI100000FF, &CPrint_NONE }, { 0x1F00FF01, 0x1F00FF01,
				0x01, &CFP_DI1F00FF0X, &CFA_DI1F00FF0X, &CPrint_NONE },
