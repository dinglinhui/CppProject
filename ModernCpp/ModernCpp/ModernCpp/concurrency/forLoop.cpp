...
int test_for_loop() {
	...
	for (auto i = start; i != 11 * start; i += start) {
		...
		auto time_rows =
				std::chrono::duration_cast<time_t>(t2_rows - t1_rows).count();
		auto time_cols =
				std::chrono::duration_cast<time_t>(t2_cols - t1_cols).count();
		auto factor = 1.0 * time_cols / time_rows;

		if (sum_rows == sum_cols) {
			std::cout << "#el = " << std::setw(9) << i * i << "  time-rows = "
					<< std::setw(6) << time_rows << "  time-cols = "
					<< std::setw(6) << time_cols << "  factor = "
					<< std::setw(6) << factor << std::endl;
		} else {
