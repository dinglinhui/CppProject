#ifndef SRC_APPLICATION_EmployeeSystem_H_
#define SRC_APPLICATION_EmployeeSystem_H_
...
public:
    ...
    void sort_print(Compare compFun) {
        std::vector<const EmployeeType*> folks;
        for (const auto& pr : m_employee)
            folks.push_back(&pr);

        std::sort(std::begin(folks), std::end(folks), [&compFun](const EmployeeType* ptr1, const EmployeeType* ptr2) -> bool {
            return compFun(ptr1, ptr2);
        });
...
#endif /* SRC_APPLICATION_EmployeeSystem_H_ */
