...
private:
    template<typename Fun, typename ... Args>
    static std::pair<typename TimeT::rep, typename std::result_of<Fun(Args...)>::type> measure_execution(Fun&& fun, Args&&... args) {
    ...
    void compute_st_dev() {
        ...
        std::transform(m_times.begin() + m_throw_away, m_times.end(), diff.begin(), [this](typename TimeT::rep t)
...
int test_benchmark() {
    ...
    for (auto numEl = n; numEl != 11 * n; numEl += n) {
        ...
        std::cout << "#elements: " << std::setw(9) << numEl << " " << " mean: " << std::setw(8) << benchmark.mean() << " st. dev: : " << std::setw(8) << benchmark.standard_deviation() << std::endl;
