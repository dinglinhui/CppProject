...
public:
	using SystemMap = std::map<SYSTEMID, std::unique_ptr<std::thread>>;
	Application() {
	}
	...
	void initSystems() {
		Subsystems_.emplace(SYSTEMID::SYSTEM_MONITOR, new MonitorSystem());
	}
