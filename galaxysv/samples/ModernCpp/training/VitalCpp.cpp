...
enum FeatureSupports {
    C99 = 0x0001, ExtInt = 0x0002, SAssert = 0x0004, NoExcept = 0x0008, SMAX = 0x0010,
};
...
template<typename T1, typename ...T> struct MutliTypes<T1, T...> : public MutliTypes<T...> {
    T1 t1;
...
template<typename I, template<typename > class A, template<typename > class ...B> struct Container<I, A, B...> {
    A<I> a;
    Container<I, B...> b;
};
...
// iterate foo: inserting into bar
    ...
    std::cout << std::endl << is_same<decltype(pf), decltype(pf1)>::value << std::endl;
